// MdnsService.h — mDNS / zeroconf service for RTNode
//
...
// (`rtnode.local`) so the captive portal can be reached by name.
//
...
// Start mDNS in AP mode for the config portal.  The hostname is fixed so
// users can always type the same .local address regardless of which device
